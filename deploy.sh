...
export SERVER_NAME='freewebistereport.info'
export GIT_REPO='git@github.com:CITGuru/websitereport.git'
export MAIN_DIR='/root/app'
export STATIC_DIR=$MAIN_DIR/static/
export PROJECT_NAME='websitereport'
export SOCK_FILE_LOC=${MAIN_DIR}/$PROJECT_NAME.sock
export SERVICE_FILE_LOC=${MAIN_DIR}/${PROJECT_NAME}.service
export TIER=master
export ENV_FILE=/root/websitereport/envfile_secrets
...
ExecStart=$VENV_BIN/gunicorn --access-logfile $HOME/${PROJECT_NAME}_access.log --error-logfile $HOME/${PROJECT_NAME}_error.log --workers 3 --bind 0.0.0.0:8713 websitereport.wsgi:application
[Install]
